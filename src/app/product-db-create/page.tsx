...
export default function AddProductPage() {
  ...
  useEffect(() => {
    setImages([]); // Ensure images are cleared when the page is loaded
  }, [setImages]); // Empty dependency array ensures this runs only once when component mounts
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formRef.current || !images) return;
    try {
      const formData = new FormData(formRef.current);
      console.log("from create page: ", images);
      images.forEach((image) => formData.append("images", image));
      console.log("Form submitted with URLs:", images);
      startTransition(() => {
        formAction(formData);
      });
    } catch (error) {
  ...
  return (
    <form
      ...
      className="bg-[#E3EFF4] p-4 space-y-4 mx-auto  "
    >
